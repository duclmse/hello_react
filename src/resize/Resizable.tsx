...
export default class Resizable extends Component<Props, void> {
  ...
  handleRefs: Map<ResizeHandleAxis, ReactRef<HTMLElement>> = new Map();
  lastHandleRect?: DOMRect = undefined;
  slack?: [number, number] = undefined;
   ...
   */
  resizeHandler(handlerName: "onResize" | "onResizeStart" | "onResizeStop", axis: ResizeHandleAxis): DraggableEventHandler {
    return (e: SyntheticEvent<>, {node, deltaX, deltaY}: DragCallbackData): void => {
  ...
  renderResizeHandle(handleAxis: ResizeHandleAxis, ref: ReactRef<HTMLElement>): ReactNode {
    ...
    return cloneElement(handle, props);
  }
    ...
    return cloneElement(children, {
      ...
      children: [
        ...
        ...resizeHandles.map(handleAxis => {
          ...
          let ref: ReactRef<HTMLElement>;
          if (this.handleRefs.has(handleAxis)) {
            ref = this.handleRefs.get(handleAxis);
          } else {
            ref = createRef<HTMLElement>();
            this.handleRefs.set(handleAxis, ref);
          }
          return (
            <DraggableCore
              {...draggableOpts}
